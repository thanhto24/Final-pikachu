...
void calcTime(const int TIME, int &timeLeft, int &prevTime, int m1, int s1, int m2, int s2, int &Point)
{
    ...
    if (timeLeft >= 0)
    {
        if (prevTime != timeLeft)
        {
            ...
            {
                TextColor(4);
                PlaySound(TEXT("Music\\timeup.wav"), NULL, SND_FILENAME | SND_ASYNC);
            }
    ...
    while (true)
    {
        ...
        if (kbhit()) // Kiểm tra xem có bấm phím kg
        {
            ...
            if (toupper(key) == 'A' || key == 75) // Di chuyển trái
            {
                ...
                if (x < 0)
                    x = m - 1;
                PlaySound(TEXT("Music\\move.wav"), NULL, SND_FILENAME | SND_ASYNC);
            }
            ...
            if (key == '!')
            {
                ...
                PlaySound(TEXT("Music\\Hint.wav"), NULL, SND_FILENAME | SND_ASYNC);
                Point -= 10;
                unSelect(n, m, ate, selected);
                for (int i = 1; i <= n; i++)
                    for (int j = 1; j <= m; j++)
            ...
            if (key == 13)
            {
                ...
                if (cnt == 1)
                {
                    ...
                    if (finalCheck(hei,wid,y1, x1, y2, x2, n, m, 1, C, view, pic, ate, movingOn, selected, cpy))
                    {
                        if (isSlide)
                        {
                            PlaySound(TEXT("Music\\match.wav"), NULL, SND_FILENAME | SND_ASYNC);
                        ...
                        {
                            PlaySound(TEXT("Music\\wrong.wav"), NULL, SND_FILENAME | SND_ASYNC);
